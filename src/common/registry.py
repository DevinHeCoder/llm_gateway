...
约定：每个可插拔层（model_client / rate_limiter / cache / monitor…）各自持有一个
Registry 实例；实现类通过 ``@registry.register("name")`` 注册；上层只按
配置里的名字调用 ``registry.create(...)`` 创建实例。

效果：新增一种实现（如新的限流算法、新的缓存后端）只需注册新类，不需要改动
上层编排代码 —— 这就是"可插拔"的落地形态。
"""
...
class Registry(Generic[T]):
    ...
    参数:
        name: 注册表名称，仅用于报错信息（如 "model_client"）。
