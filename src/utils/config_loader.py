...
典型用法::
    ...
    api_key = cfg["model"]["api_key"]      # 密钥来自环境变量 GATEWAY_MODEL_API_KEY

覆盖机制（优先级从低到高）:
    1. YAML 文件本体
    2. ${ENV_VAR} / ${ENV_VAR:default} 占位符（用于注入密钥、地址等）
    3. 环境变量路径覆盖：GATEWAY_MODEL__MODEL=x 覆盖 cfg["model"]["model"]
...
def _apply_env_overrides(cfg: Dict[str, Any], prefix: str) -> Dict[str, Any]:
    """环境变量路径覆盖：GATEWAY_MODEL__MODEL=qwen2.5 覆盖 cfg['model']['model']。
